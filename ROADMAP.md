...
1. **`AgentRuntimeAdapter` phase 1.** Warren-only, parallel with the campaign — the file sets are disjoint. The core event-envelope extractor and the `usageShape` usage-reader consolidation landed in v0.14.0 (warren-27b5, warren-db2e). Remaining: move `classifyTerminalProviderError` (`src/runs/reap/provider-error.ts`) and `HARNESS_STATE_PREFIXES` (`src/runs/reap/util.ts`) behind the adapter registry. The earlier `seedLayout` target name never existed as a symbol — per-runtime workspace layout arrives with the registry itself. Type `runtimeId` off `KNOWN_RUNTIME_IDS` at the contract seam, where `src/runtime/contract.ts` still says `string`. Enforcement: a runtime-id literal outside the adapter directory fails lint. Published as good-first-challenge GH#846 (2026-08-13). Named non-goals: burrow's `AgentRuntime` interface, the k8s in-pod dispatcher, steering encoding — all of it phase 2 scope (item 3).
