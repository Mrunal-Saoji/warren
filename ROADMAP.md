...
| Forge | `Forge` — repo refs, git auth, PR open/find, checks, error taxonomy | `now` — owner go 2026-08-11, plan pl-d1c9 in flight. Design record: `docs/design/forge-contract.md`. Paid by App installation tokens via the manifest flow. |
| Issue tracker | `IssueTracker` — capability-flagged (`supportsPlans`). External implementations arrive through the `RemoteTracker` bridge (Decisions, 2026-08-04). | `next`, after the Forge campaign — paid by Linear, the first external tracker extension. |
...
- **The Forge campaign (Next item 1), as plan pl-d1c9.** Owner go recorded 2026-08-11. [`docs/design/forge-contract.md`](docs/design/forge-contract.md), amended the same day from a full audit of HEAD, is the design record. Umbrella seed warren-3fe7 tracks nineteen forward-chained children across the five phases. Falsification tests: a FakeForge project completes dispatch → reap → push → PR with zero domain-code changes, and scenario 39 stays green at every commit.

## Next — planned, in order

1. **The Forge campaign: `Forge` paid by GitHub App installation tokens.** In flight — see Now (plan pl-d1c9, owner go 2026-08-11). Descoped from the old GitHub App campaign on 2026-08-03 — the login half moved to Deferred until paid (see Decisions). The design doc is [`docs/design/forge-contract.md`](docs/design/forge-contract.md). Internal order: consolidate the four GitHub REST clients into `src/forge/github/` first. The 2026-08-08 audit counted four clients, not three, and the naive union exceeds the file-size budget, so the tree arrives decomposed. Then cut a capability-minimal `Forge` contract, with FakeForge as implementation #2, gated in the same PR. Then the credential story, once: the forge mints and refreshes its own tokens. Two mechanisms back it — a fine-grained PAT, and installation tokens from a GitHub App that each deployment registers through the manifest flow. PAT mode is a permanent peer, never a legacy path. Hourly-expiring installation tokens are the forcing function. A static token threaded through 66 non-test files cannot express them, so the App pays for the consolidation. The audit corrected the old ~30 count, which missed the handler, k8s, and acceptance threading. Falsification tests: a FakeForge project completes dispatch → reap → push → PR with zero domain-code changes, and scenario 39 stays green at every commit.
2. **`AgentRuntimeAdapter` phase 1.** Warren-only, parallel with the campaign — the file sets are disjoint. The core event-envelope extractor and the `usageShape` usage-reader consolidation landed in v0.14.0 (warren-27b5, warren-db2e). Remaining: move `classifyProviderError`, `seedLayout`, and `harnessStatePrefixes` behind the adapter registry. Type `runtimeId` off `KNOWN_RUNTIME_IDS`. Enforcement: a runtime-id literal outside the adapter directory fails lint. Named non-goals: burrow's `AgentRuntime` interface, the k8s in-pod dispatcher, steering encoding — all of it phase 2 scope (item 4).
3. **`IssueTracker` — the contract cut, then the bridge.** After the Forge campaign, which still pays for the GitHub HTTP core and the credential plumbing on the forge side. The 2026-08-04 decision reshapes the implementation plan: trackers arrive as extensions (see Decisions). First cut the TypeScript contract: unfuse seeds from dispatch prompt-building, the reap close hook, the plan-run coordinator, and the `/projects/:id/seeds/*` routes. Seeds becomes implementation #1 behind the seam. Then build `RemoteTracker`, the bridge that speaks `warren-tracker/v1` to an external container. A published conformance suite is the falsification test. The bridge design consumes the pl-116e friction report for its packaging and handshake conventions. Linear ships as the first external tracker extension. GitHub Issues follows as an extension too, not on the shared HTTP core. Re-derived blockers: credential storage (B1) dissolves, because the extension holds its own tracker token. Caching and backoff (B3, B4) move into the extension. FakeTracker (B5) becomes the conformance suite. The sandbox stays `network: none` (B6): tracker calls proxy through warren, which forwards to the bridge. The public-mode leak (B2) stays a contract-time owner decision. Design input from the deleted pl-fb43 prototype: any future cross-repo routing must key off tracker-agnostic metadata. That means the 2026-07-29 sidecar-table amendment, keyed by (project_id, issue_id) — never a tracker-specific field like seeds extensions.repo.
