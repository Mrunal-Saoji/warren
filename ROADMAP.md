...
| Issue tracker | `IssueTracker` — capability-flagged (`supportsPlans`). External implementations arrive through the `RemoteTracker` bridge (Decisions, 2026-08-04). | `next` — unblocked by the Forge campaign shipping in v0.15.0; paid by Linear, the first external tracker extension. |
...
- Nothing mid-campaign. The agent-analytics campaign (pl-103e) and the judge extension (pl-17ca) both shipped in v0.16.0 — see Shipped. Most of `AgentRuntimeAdapter` phase 1 also landed in v0.16.0: PR #887 shipped the adapter registry and both tenant moves (GH#846 items 1–3). Two slices remain: type `runtimeId` off the union at the contract seam, and add the lint guard (items 4–5). The self-host push (Next items 2–4) queues next, per the 2026-08-16 operator decision. The `IssueTracker` contract cut follows it.

## Next — planned, in order

1. **`AgentRuntimeAdapter` phase 1.** Warren-only, parallel with the campaign — the file sets are disjoint. The core event-envelope extractor and the `usageShape` usage-reader consolidation landed in v0.14.0 (warren-27b5, warren-db2e). The adapter registry (`src/runtime/adapters/`) landed in v0.16.0 (PR #887). It moved `classifyTerminalProviderError`'s envelope types and `HARNESS_STATE_PREFIXES` behind per-adapter declarations. Both call sites read the union across adapters, which preserves behavior. A narrower per-run read needs `runtimeId` on the runs row first. Remaining: type `runtimeId` off `KNOWN_RUNTIME_IDS` at the contract seam, where `src/runtime/contract.ts` still says `string`. Enforcement: a runtime-id literal outside the adapter directory fails lint. Published as good-first-challenge GH#846 (2026-08-13). Named non-goals: burrow's `AgentRuntime` interface, the k8s in-pod dispatcher, steering encoding — all of it phase 2 scope (item 2).
2. **`AgentRuntimeAdapter` phase 2 — repatriate the harness logic from burrow.** Promoted from Deferred on 2026-07-30. The promotion trigger was an explicit decision to cut the k8s dependency on `@os-eco/burrow-cli`, and the owner made that decision (see Decisions). Move `AgentRegistry`, the pi and claude-code parsers, `buildSpawnCommand`, and the steering encoders into the warren adapter registry — a source lift from burrow, not a rewrite. (The old clause to rehome the ~15 domain files' burrow types is done. The 2026-07-16/17 burrow-client eviction commits cleared the domain-level type leakage. `check:layers` holds the boundary — verified against HEAD 2026-08-13.) Exit criterion: the k8s path (`src/runtime/k8s/`) imports zero burrow code. The npm dependency survives only for the LocalProvider daemon client. Depends on phase 1 only. The file sets are disjoint from the campaign, so this phase can run early if capacity allows.
3. **Sandbox internalization — the burrow endgame.** `LocalProvider` spawns agents through warren-owned bwrap / sandbox-exec profile generation (lifted from burrow), driven by the same host-side loop the k8s entrypoint already runs. Kills `burrow serve`, the unix socket, the token handshake, `src/burrow-client/`, and the supervisor sibling process. No intermediate raw-exec daemon mode — the absorption decision retires that contract. The excision rides with it: drop `@os-eco/burrow-cli` from `package.json` and the Dockerfile, the burrow-pin assertions in `check:version-sync`, and the two burrow rules in `layer-rules.json`. Rewrite the CLAUDE.md and docs/design/runtime-and-supervisor.md burrow sections. The compose bwrap capability flags stay — warren itself runs bwrap after this.
4. **One-line docker self-host.** The 2026-08-16 self-host push: the home-server install is the headline pitch, and today's quickstart falsifies it. The exit criterion is one `docker run` with two secrets (`ANTHROPIC_API_KEY`, `GITHUB_TOKEN`), no security flags, and no burrow tokens. Three pieces pay for it. Mint `WARREN_API_TOKEN` on first boot when unset and print it to the logs. Mint the burrow channel token inside the supervisor, because both ends of that channel live in the one container it spawns. Add a `DockerProvider` that runs each agent in a sibling container over the docker socket. That ports the k8s pod-boundary model to the local topology, and it makes nested bwrap and the four compose flags optional. The two token wins can ship before the excision. Pin the path with an `acceptance:container` scenario: fresh host, one command, dispatch succeeds.
5. **`IssueTracker` — the contract cut, then the bridge.** The Forge campaign (shipped v0.15.0) paid for the GitHub HTTP core and the credential plumbing on the forge side. The 2026-08-04 decision reshapes the implementation plan: trackers arrive as extensions (see Decisions). First cut the TypeScript contract: unfuse seeds from dispatch prompt-building, the reap close hook, the plan-run coordinator, and the `/projects/:id/seeds/*` routes. Seeds becomes implementation #1 behind the seam. Then build `RemoteTracker`, the bridge that speaks `warren-tracker/v1` to an external container. A published conformance suite is the falsification test. The bridge design consumes the pl-116e friction report for its packaging and handshake conventions. Linear ships as the first external tracker extension. GitHub Issues follows as an extension too, not on the shared HTTP core. Re-derived blockers: credential storage (B1) dissolves, because the extension holds its own tracker token. Caching and backoff (B3, B4) move into the extension. FakeTracker (B5) becomes the conformance suite. The sandbox stays `network: none` (B6): tracker calls proxy through warren, which forwards to the bridge. The public-mode leak (B2) stays a contract-time owner decision. Design input from the deleted pl-fb43 prototype: any future cross-repo routing must key off tracker-agnostic metadata. That means the 2026-07-29 sidecar-table amendment, keyed by (project_id, issue_id) — never a tracker-specific field like seeds extensions.repo.
